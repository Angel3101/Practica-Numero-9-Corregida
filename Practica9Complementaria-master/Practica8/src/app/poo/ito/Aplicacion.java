...
public class Aplicacion {
	...
	static void menu() throws HeadlessException, FileNotFoundException {
		inicializa();
		iniciaPrograma();
	...
	static Cuenta_Bancaria capturarCuenta() throws Exception{
		...
		nombre=JOptionPane.showInputDialog("Mencione nombre del cliente (NombreApellido):");
		saldo=Float.parseFloat(JOptionPane.showInputDialog("Mencione saldo de la cuenta:"));
		fecha=JOptionPane.showInputDialog("Mencione fecha de la apertura(aaaa-mm-dd):");
		n.setNumCuenta(l);
	...
	static void hacerDeposito() throws HeadlessException, FileNotFoundException  {
		...
		if(c.isFree())
		    ...
		    for(int i=0;i<c.getSize();i++)
			    cuentas=cuentas+"\n"+(i+1)+")"+(c.getItem(i));
		    pos=Integer.parseInt(JOptionPane.showInputDialog("Mencione la cuenta al cual desee hacer un deposito\n"+cuentas));
		    if((c.getSize())>=pos&&pos>0) {
		    cantidad=Float.parseFloat(JOptionPane.showInputDialog("Cuanta cantidad desea depositar?"));
		    c.HeadlessException (cantidad);
	...
	static void hacerRetiro() throws HeadlessException,FileNotFoundException  {
		...
		if(c.isFree())
		    ...
		    for(int i=0;i<c.getSize();i++)
			    cuentas=cuentas+"\n"+(i+1)+")"+(c.getItem(i));
		    pos=Integer.parseInt(JOptionPane.showInputDialog("Mencione la cuenta al cual desee hacer un retiro\n"+cuentas));
		    if((c.getSize())>=pos&&pos>0) {
		    ...
		    bandera=false;
		    	...
		    	JOptionPane.showMessageDialog(null,"La cantidad sobrepasa el saldo de la cuenta!");
		    }
		    }
		    else
		    	JOptionPane.showMessageDialog(null,"Esa cuenta no es existente");
	...
	static void borrarCuenta() throws HeadlessException, FileNotFoundException  {
		...
		if(c.isFree())
		    ...
		    if((c.getSize())>=pos&&pos>0) {
		    	c.HeadlessException (c.getItem(pos-1));
		    	c.clear(c.getItem(pos-1));
		    	JOptionPane.showMessageDialog(null,"¡La cuenta se ha dado de baja satisfactoriamente!");
		    	bandera=false;
		    }	
		    else
		    	JOptionPane.showMessageDialog(null,"Esa cuenta no es existente");
	...
	static void mayor10mil() {
		if(c.isFree())
			...
			for(int j=0;j<(c.getSize()-vacio);j++)
				cuentas=cuentas+"\n"+copia[j];
			JOptionPane.showMessageDialog(null,"Las cuentas que tienen saldo que sobrepasan a $10,000 son:\n"+cuentas);
		}
	}
	...
	static void iniciaPrograma() throws Exception  {
		...
		try {
			...
			System.err.println("No existe un archivo antecedente, se hara la realizacion en proceso de uno nuevo");
		}
		if(existe)
			leerTXT();
